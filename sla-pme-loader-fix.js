(function(){
'use strict';
if(window.__SLA_PME_LOADER_FIX_V2__)return;
window.__SLA_PME_LOADER_FIX_V2__=true;
...
async function getClient(){
 if(!window.supabase)throw new Error('supabase.min.js não carregou.');
 const key=cfg.SUPABASE_PUBLISHABLE_KEY||cfg.SUPABASE_ANON_KEY;
 if(!cfg.SUPABASE_URL||!key)throw new Error('portal-config.js não possui URL/chave do Supabase.');
 const client=window.__SLA_SUPABASE_CLIENT__||(window.__SLA_SUPABASE_CLIENT__=window.supabase.createClient(cfg.SUPABASE_URL,key,{auth:{persistSession:true,autoRefreshToken:true,detectSessionInUrl:true}}));
